...
# def RandomCrop(img, bboxes_xyxy, classes, w_constraint=2, h_constraint=2, iou_constraint=0.5, p=1.0):
#     if random.random() < p:
...
#     while(True):
#         img = cv2.imread("000005.jpg", cv2.IMREAD_COLOR)

#         import dataset
#         label = np.loadtxt("000005.txt",
#                                   dtype=np.float32,
...
# # def RandomScale(img, bboxes_xyxy, classes, p=1.0, threshold_w=64, threshold_h=64):
...
# # def drawBBox(img, bboxes_xyxy, valid_bboxes_mask):
# #     h, w = img.shape[:2]

# #     bboxes_xyxy[:, [0, 2]] *= w
# #     bboxes_xyxy[:, [1, 3]] *= h

# #     for bbox_xyxy, valid in zip(bboxes_xyxy, valid_bboxes_mask):
# #         print(bbox_xyxy)
...
# # def RandomErase(img, bboxes_xyxy, p=1.0):
# #     if random.random() < p:
# #         img_h, img_w = img.shape[0:2]
...
# #             for _ in range(10):
# #                 cropped_w = random.randint(0, w)
# #                 cropped_h = random.randint(0, h)

# #                 if (cropped_w * cropped_h) / (w * h) > 0.3:
# #                     continue
                
# #                 cropped_xmin = random.randint(xmin, xmax-cropped_w)
# #                 cropped_ymin = random.randint(ymin, ymax-cropped_h)

# #                 cropped_xmax = cropped_xmin + cropped_w
# #                 cropped_ymax = cropped_ymin + cropped_h
            
# #                 img[cropped_ymin:cropped_ymax, cropped_xmin:cropped_xmax] = 127
# #                 break
...
# # # def RandomCrop(img, bboxes_xyxy, classes, w_constraint=3, h_constraint=3, iou_constraint=0.5, p=1.0):
# # #     if random.random() < p:
# # #         img_h, img_w = img.shape[0:2]
...
# # #         min_cropped_img_w = np.minimum(int(np.round(np.min(bboxes_w) * 1.5)), img_w - 1)
# # #         min_cropped_img_h = np.minimum(int(np.round(np.min(bboxes_h) * 1.5)), img_h - 1)
        
# # #         for _ in range(50):
# # #             cropped_img_w = random.randint(min_cropped_img_w, img_w)
# # #             cropped_img_h = random.randint(min_cropped_img_h, img_h)

# # #             if np.max([cropped_img_w, cropped_img_h])/np.min([cropped_img_w, cropped_img_h]) > 2.0:
# # #                 continue
...
# # #             size_mask = (cropped_bboxes_w >= w_constraint) & (cropped_bboxes_h >= h_constraint)
# # #             if np.count_nonzero(size_mask) == 0:
# # #                 continue
            
# # #             iou = cropped_bboxes_area[size_mask]/bboxes_area[size_mask]
# # #             if np.count_nonzero(iou < iou_constraint) > 0: #iou_constraint 퍼센트 이상 가려진 물체가 있으면 다시 RandomCrop, 너무 많이 Crop된걸 찾도록 학습시키면 상식적으로 이상혀~ 비주얼라이제이션 해보면 알음
# # #                 continue

# # #             img = img[t:b, l:r]

# # #             cropped_bboxes_xyxy = cropped_bboxes_xyxy[size_mask]
# # #             cropped_bboxes_xyxy[:, [0, 2]] -= l
# # #             cropped_bboxes_xyxy[:, [1, 3]] -= t

# # #             cropped_bboxes_xyxy = cropped_bboxes_xyxy.astype(np.float32)
# # #             cropped_bboxes_xyxy[:, [0, 2]] /= cropped_img_w
# # #             cropped_bboxes_xyxy[:, [1, 3]] /= cropped_img_h
# # #             cropped_bboxes_xyxy = np.clip(cropped_bboxes_xyxy, 0, 1.)
...
# # # def RandomCrop(img, bboxes_xyxy, classes, w_constraint=3, h_constraint=3, p=1.0):
# # #     if random.random() < p:
# # #         img_h, img_w = img.shape[0:2]
...
# # #             if np.count_nonzero(iou_mask) == 0:
# # #                 continue

# # #             img = img[t:b, l:r]

# # #             bboxes_xyxy = cropped_bboxes_xyxy[size_mask][iou_mask]
# # #             bboxes_xyxy[:, [0, 2]] -= l
# # #             bboxes_xyxy[:, [1, 3]] -= t

# # #             bboxes_xyxy = bboxes_xyxy.astype(np.float32)
# # #             bboxes_xyxy[:, [0, 2]] /= cropped_img_w
# # #             bboxes_xyxy[:, [1, 3]] /= cropped_img_h
# # #             bboxes_xyxy = np.clip(bboxes_xyxy, 0, 1.)

# # #             classes = classes[size_mask][iou_mask]
# # #             break
# # #     return img, bboxes_xyxy, classes
...
# #             iou = cropped_bboxes_area[size_mask]/bboxes_area[size_mask]
# #             iou_mask = iou > iou_constraint
            
# #             if np.count_nonzero(iou_mask) == 0: #iou_constraint 퍼센트 이상 가려진 물체가 있으면 다시 RandomCrop, 너무 많이 Crop된걸 찾도록 학습시키면 상식적으로 이상혀~ 비주얼라이제이션 해보면 알음
# #                 continue
            
# #             valid_bboxes_mask = iou_mask
...
# #             return img, cropped_bboxes_xyxy, classes, valid_bboxes_mask
# #     return img, bboxes_xyxy, classes, np.ones_like(classes)
...
# #         valid_bboxes_mask = np.ones_like(classes)
...
# # #        img, bboxes_xyxy = RandomErase(img, bboxes_xyxy)
...
# #         drawBBox(img, bboxes_xyxy, valid_bboxes_mask)
# #         cv2.imshow("img", img)
# #         ch = cv2.waitKey(0)
